...
This repo is a reusable module. Product repositories should pin a released tag
or commit in their own `deps.json` and consume public headers from `deps/`.

Recent workspace maintenance added:

- `docs/module_structure.md` for the module contract.
- `scripts/audit_packet_buffers.sh` and `make -f Makefile.linux packet-buffer-audit`.
- `scripts/test_with_testkit.sh` and `make -f Makefile.linux testkit-wrapper`.
- Existing repeatable P2P benchmarks in `scripts/bench_p2p_scale.sh` and
...
TODO state is tracked in `docs/todo.yaml`; `docs/todo.md` is retained as legacy
history.
